...
export default function GoogleCallback() {
  ...
  const handleError = (msg) => {
    setStatus('error');
    setMessage(msg);
    // Tenta fechar a janela após 2 segundos
    setTimeout(() => {
  ...
  async function processLogin(code) {
    try {
      ...
      // Reaproveitamos a mesma rota de callback do backend, pois ela só precisa do "code"
      ...
      if (window.opener) {
        ...
        // Usamos uma chave diferente para o Google saber que terminou
        window.opener.sessionStorage.setItem('google_auth_result', 'success');
